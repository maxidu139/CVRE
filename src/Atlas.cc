...
void Atlas::PreSave()
{
	cout << "Linea 303:PreSave()" << endl;
    if(mpCurrentMap)
    {
	cout << "Linea 306:PreSave()" << endl;
        if(!mspMaps.empty() && mnLastInitKFidMap < mpCurrentMap->GetMaxKFid())
        {
		cout << "Linea 309:PreSave()" << endl;
            mnLastInitKFidMap = mpCurrentMap->GetMaxKFid()+1; //The init KF is the next of current maximum
    	}
    }
    		cout << "Linea 313:PreSave()" << endl;
    struct compFunctor
    {
        inline bool operator()(Map* elem1 ,Map* elem2)
        {
		...
		cout << "Linea 321:PreSave()" << endl;
    std::copy(mspMaps.begin(), mspMaps.end(), std::back_inserter(mvpBackupMaps));
    sort(mvpBackupMaps.begin(), mvpBackupMaps.end(), compFunctor());

    std::set<GeometricCamera*> spCams(mvpCameras.begin(), mvpCameras.end());
		cout << "Linea 326:PreSave()" << endl;
    for(Map* pMi : mvpBackupMaps)
    {
	cout << "Linea 329:PreSave()" << endl;
        if(pMi == NULL)
        {
            continue;
    	}
	...
	cout << "Linea 338:PreSave()" << endl;
        if(pMi->GetAllKeyFrames().size() == 0) {
	cout << "Linea 340:PreSave()" << endl;
            // Empty map, erase before of save it.
            SetMapBad(pMi);
	cout << "Linea 343:PreSave()" << endl;
            continue;
        }
	cout << "Linea 346:PreSave()" << endl;
        pMi->PreSave(spCams);
	cout << "Linea 348:PreSave()" << endl;
    }
    		cout << "Linea 345:PreSave()" << endl;
    RemoveBadMaps();
}
